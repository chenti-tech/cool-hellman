...
// 資料庫路徑使用的 App ID (請勿隨意更改，以免讀取不到舊資料)
const appId = "chenti-essay-system-v1";
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // 簡化的驗證流程：如果沒有使用者登入，則嘗試匿名登入以讀取公開資料
    const unsubscribe = onAuthStateChanged(auth, (u) => {
      ...
      if (u) {
        ...
        signInAnonymously(auth).catch((err) => {
          ...
          // 即使失敗也允許進入登入畫面，讓使用者可以嘗試 Email 登入
          if (view === "loading") setView("login");
        });
      }
    });
  ...
  const handleLogout = () => {
    ...
    setView("login");
    // 如果是管理員登出，我們可以選擇是否要登出 Firebase Auth
    // 但為了保持匿名連線讀取，這裡僅切換 View 狀態即可
  ...
  if (!auth) {
    return (
      <div className="flex h-screen items-center justify-center bg-gray-50 flex-col p-4 text-center">
        ...
        <p className="text-gray-700 mb-4">請檢查您的網路連線或配置設定。</p>
      </div>
    );
  }
